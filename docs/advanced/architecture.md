...
This question is best answered by tracing what happens when a user (you!) runs `bun run quartz build` in the command line:

## On the server

1. After running `bun run quartz build`, Bun will look at `package.json` to find the `bin` entry for `quartz` which points at `./quartz/bootstrap-cli.mjs`.
2. This file has a [shebang](<https://en.wikipedia.org/wiki/Shebang_(Unix)>) line at the top which tells Bun to execute it using Bun.

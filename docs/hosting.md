...
> [!warning]
> Cloudflare Pages performs a shallow clone by default, so if you rely on `git` for timestamps, it is recommended that you add `git fetch --unshallow &&` to the beginning of the build command (e.g., `git fetch --unshallow && bun run quartz build`).
...
2. Commit these changes by doing `bun run quartz sync`. This should deploy your site to `<github-username>.github.io/<repository-name>`.
...
>
> Make sure you save your changes to Git and sync it to GitHub by doing `bun run quartz sync`. This will also make sure to pull any updates you may have made from other devices so you have them locally.
...
| Root Directory                            | `./`                   |
| Build and Output Settings > Build Command | `bun run quartz build` |
...
3. Under "Build command", enter `bun run quartz build`.
